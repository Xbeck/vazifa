...
from app.schemas.user_schemas import UserChangePasswordRequest, UserEditRequest, MyUserResponse, UserResponse
...
async def get_users(user_id: Optional[int] = None, db: AsyncSession = Depends(get_database)) -> Union[Sequence[User], User]:
...
async def get_my_user(request: Request, db: AsyncSession = Depends(get_database)) -> User:
...
async def make_admin(user_id: int, db: AsyncSession = Depends(get_database)) -> None:
...
async def ban_user(request: Request, user_id: int, db: AsyncSession = Depends(get_database)) -> None:
    """Ban the specific user Id. | Admins only. The Admin cannot ban their own ID!"""
...
async def unban_user(request: Request, user_id: int, db: AsyncSession = Depends(get_database)) -> None:
    """Ban the specific user Id. | Admins only."""
...
async def edit_user(user_id: int, user_data: UserEditRequest, db: AsyncSession = Depends(get_database)) -> Union[User, None]:
    """Update the specified User's data. | Available for the specific requesting User, or an Admin."""
...
async def delete_user(user_id: int, db: AsyncSession = Depends(get_database)) -> None:
    """Delete the specified User by user_id | Admin only."""
